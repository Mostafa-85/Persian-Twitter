...
#
#             user = authenticate(request, username=username, password=password,last_login=now())
#             if user is not None:
#                 login(request, user)
#                 # messages.success(request, "You have successfully logged in.")
#                 return redirect('home_page')
#             else:
#                 messages.error(request, "نام کاربری یا رمزعبور نامعتبر است")
#
#
#     return render(request, 'Entrance section/login.html', {'form': form})
#
...
#             username=username, email=email, password=password,date_joined=now(),last_name=last_name,first_name=first_name)
...
#
#     return render(request,'Entrance section/Registration.html',{'form':register})
#
#
...
#         # جلوگیری از نمایش دکمه‌ها در صورت مشاهده پروفایل خود کاربر
#         is_own_profile = user_profile == profile
#         # بررسی فالو بودن
#         is_following = Connection.objects.filter(from_user=user_profile, to_user=profile).exists()
#
#     context = {
#         'profile': profile,
#         'followers': followers,
...
#     }
#     return render(request, 'twitt/user/profile_detail.html', context)
#
# def follow_user_view(request,pk):
#     if request.user.is_authenticated:
#         from_user = request.user.userprofile
#         to_user = get_object_or_404(UserProfile, pk=pk)
#         if from_user != to_user:
#             Connection.objects.get_or_create(from_user=from_user, to_user=to_user)
